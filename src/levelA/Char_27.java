...
public class Char_27 {
    private void solve() {

        String strFirst = nextToken().toLowerCase(Locale.ROOT);
        String strSecond = nextToken().toLowerCase(Locale.ROOT);

        for (int i = 0; i < strFirst.length(); i++) {
            if (strFirst.charAt(i) < strSecond.charAt(i)) {
                System.out.println("-1");
                break;
            } else if (strFirst.charAt(i) > strSecond.charAt(i)) {
                System.out.println("1");
                break;
            } else if (i == strFirst.length() - 1) {
